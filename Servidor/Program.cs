...
doc.MapDelete("/gastos/{gastoID:int}", async (int gastoID, [FromQuery(Name = "presupuestos")] int[]? presupuestos, rDocumentos repo) =>
{
    try
        {
        var presupuestosList = presupuestos?.Distinct().ToList();
        var result = await repo.BorrarGastoAsync(gastoID, presupuestosList);
        return Results.Ok(new
            {
            Success = true,
            DocumentoID = result?.DocumentoID ?? 0,
            PresupuestoIDs = result?.PresupuestoIDs ?? new List<int>(),
            Resumenes = result?.Resumenes ?? new List<rDocumentos.PresupuestoResumen>(),
